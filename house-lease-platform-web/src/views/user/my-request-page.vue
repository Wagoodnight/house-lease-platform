      ...
      <el-table-column fixed="right" label="操作" :min-width="MyTableColumn.rightWidth">
        <template #default="scope">
          ...
          <el-button
              ...
              type="primary"
              size="small"
              @click.prevent="viewEditorDetails(scope.row.requestId)"
          >
            编辑
          </el-button>
          <el-button
...
const fetchTableData = async () => {
  try {
    const result = await userApi.myRequestPage(postData.value);
  ...
  formEl.validate(async valid => {
    if(valid) {
      ElMessageBox.confirm(
          ...
          .then(async () => {
            await userApi.updateRequest(updateData.value);
            formEl.resetFields()
            await handleClick()
            ElMessage({
              type: 'success',
